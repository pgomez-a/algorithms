...
### Algorithm Analysis
In Computer Science, the analysis of algorithms is all about finding and studying the complexity of them (amount of time, storage, resources, etc. thar are needed for their execution time). Typically, this involves using a function that relates the length of the input to the number of steps that the algorithm needs to run (temporal complexity) or the memory storage it takes (spatial complexity). The algorithm is said to be efficient if the function returns low outputs or if they grow slowly compared to the growth of the input values. However, different inputs have different outputs, so it is a good idea to consider the best-case, worst-case and average-case.
